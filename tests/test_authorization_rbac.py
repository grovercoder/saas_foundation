...
def auth_manager_rbac(mock_logger):
    ...
    manager.register_permissions(
        [
            {
                "key": "manage_subscription_tiers",
                "action": "manage",
                "resource": "subscription_tier",
                "scope": "global",
            },
            {
                "key": "manage_own_users",
                "action": "manage",
                "resource": "user",
                "scope": "own",
            },
            ...
            {
                "key": "manage_all_incident_reports",
                "action": "manage",
                "resource": "incident_report",
                "scope": "global",
            },
            {
                "key": "create_incident_report",
                "action": "create",
                "resource": "incident_report",
                "scope": "global",
            },
            {
                "key": "edit_own_incident_report",
                "action": "edit",
                "resource": "incident_report",
                "scope": "own",
            },
            {
                "key": "edit_incident_report_55",
                "action": "edit",
                "resource": "incident_report",
                "id": 55,
            },
...
def test_saas_admin_can_manage_subscription_tiers(auth_manager_rbac):
    ...
    assert (
        auth_manager_rbac.is_authorized(user_roles, "manage", "subscription_tier")
        is True
    )
    assert (
        auth_manager_rbac.is_authorized(user_roles, "manage", "user") is False
...
def test_account_admin_can_manage_own_users(auth_manager_rbac):
    ...
    assert (
        auth_manager_rbac.is_authorized(
            user_roles, "manage", "user", resource_id=1, resource_owner_id=1, user_id=1
        )
        is True
    )
    ...
    assert (
        auth_manager_rbac.is_authorized(
            user_roles, "manage", "user", resource_id=2, resource_owner_id=1, user_id=2
        )
        is False
    )
    # No user_id provided
    assert (
        auth_manager_rbac.is_authorized(
            user_roles, "manage", "user", resource_id=1, resource_owner_id=1
        )
        is False
    )
...
def test_safety_officer_can_manage_all_incident_reports(auth_manager_rbac):
    ...
    assert (
        auth_manager_rbac.is_authorized(
            user_roles, "manage", "incident_report", resource_id=1
        )
        is True
    )
    assert (
        auth_manager_rbac.is_authorized(
            user_roles, "manage", "incident_report", resource_id=999
        )
        is True
    )
...
def test_normal_user_can_create_incident_reports(auth_manager_rbac):
    user_roles = ["Normal_User"]
    assert (
        auth_manager_rbac.is_authorized(user_roles, "create", "incident_report") is True
    )
...
def test_special_x_can_edit_incident_report_55(auth_manager_rbac):
    ...
    assert (
        auth_manager_rbac.is_authorized(
            user_roles, "edit", "incident_report", resource_id=55
        )
        is True
    )
    assert (
        auth_manager_rbac.is_authorized(
            user_roles, "edit", "incident_report", resource_id=56
        )
        is False
    )
    assert (
...
def test_user_with_multiple_roles(auth_manager_rbac):
    ...
    assert (
        ...
        is False
    )  # SAAS Admin doesn't get Account Admin perms
