...
def test_register_permissions_duplicate_key(auth_manager):
    ...
    auth_manager.logger.warning.assert_any_call(
        "Permission with key 'duplicate:key' already registered. Skipping."
    )
    assert len(auth_manager.get_registered_permissions()) == 1


def test_register_permissions_missing_fields(auth_manager):
    permissions = [{"name": "Invalid"}]  # Missing key
    auth_manager.register_permissions(permissions)
    auth_manager.logger.warning.assert_any_call(
        "Attempted to register permission with missing fields (key, name, or description): {'name': 'Invalid'}"
    )
...
def test_register_permissions_invalid_key_format(auth_manager):
    ...
    auth_manager.register_permissions(permissions)
    auth_manager.logger.warning.assert_any_call(
        "Attempted to register permission with invalid key format: {'key': 'invalidkeyformat', 'name': 'Invalid', 'description': 'Invalid key'}"
    )
    assert len(auth_manager.get_registered_permissions()) == 0

    permissions = [{"key": 123, "name": "Invalid", "description": "Invalid key type"}]
    auth_manager.register_permissions(permissions)
    auth_manager.logger.warning.assert_any_call(
        "Attempted to register permission with invalid key format: {'key': 123, 'name': 'Invalid', 'description': 'Invalid key type'}"
    )
    assert len(auth_manager.get_registered_permissions()) == 0
