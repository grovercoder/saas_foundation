...
class BaseDAO:
    ...
    def insert(self, data):
        ...
        cursor = execute_query(query, tuple(data.values()), conn=self.connection)
        return cursor.lastrowid

    def get_by_id(self, int_id):
        query = f"SELECT * FROM {self.table_name} WHERE id = ?"
        row = fetch_one(query, (int_id,), conn=self.connection)
        if row:
            return dict(row)
        return None
    ...
    def get_all(self):
        ...
        rows = fetch_all(query, conn=self.connection)
    ...
    def update(self, int_id, data):
        ...
        execute_query(query, tuple(list(data.values()) + [int_id]), conn=self.connection)

    def delete(self, int_id):
        query = f"DELETE FROM {self.table_name} WHERE id = ?"
        execute_query(query, (int_id,), conn=self.connection)

    def find_one_by_column(self, column_name, value):
        query = f"SELECT * FROM {self.table_name} WHERE {column_name} = ?"
        row = fetch_one(query, (value,), conn=self.connection)
        if row:
            return dict(row)
        return None
    ...
    def find_by_column(self, column_name, value):
        query = f"SELECT * FROM {self.table_name} WHERE {column_name} = ?"
        rows = fetch_all(query, (value,), conn=self.connection)
        return [dict(row) for row in rows]
